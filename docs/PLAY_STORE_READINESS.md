...
FerryFile - Android-приложение, которое запускает на телефоне локальный HTTP-сервер для обмена файлами с компьютером через браузер.
...
1. Передача PIN, cookie и файлов выполняется по открытому HTTP.
...
3. Серверные сессии отзываются при logout и остановке сервера, но не имеют серверного TTL; cookie не настроена для HTTPS-сценария.
...
- `EncryptedSharedPreferences` для порта, темы и SAF URI.
...
- `app/src/main/java/ru/kryu/ferryfile/domain/model/ServerAddress.kt` формирует URL с `http://`.
- `app/src/main/java/ru/kryu/ferryfile/server/KtorServer.kt:39` слушает `0.0.0.0`.
- PIN, session cookie и содержимое файлов могут быть перехвачены другим участником сети.

Рекомендуемое решение:

- Добавить HTTPS и понятный сценарий доверия к локальному сертификату.
- Показывать пользователю fingerprint сертификата или другой способ подтверждения pairing.
- Указать в privacy policy, как и куда передаются файлы.
- Если HTTPS откладывается, добавить заметное предупреждение о незашифрованном HTTP и ограничить сценарий доверенной локальной сетью. Это компромисс, а не полноценное решение.
...
- Cookie задаёт только `HttpOnly` и `Path`; нет явных `Secure`, `SameSite` и согласованного срока жизни. `Secure` можно включать только после перехода на HTTPS.
...
- После перехода на HTTPS включить `Secure` и настроить `SameSite`.
...
- передачу файлов между телефоном и браузером по локальной сети только по явному действию пользователя; текущий транспорт - незашифрованный HTTP;
...
- [ ] Принять решение по HTTPS и модели pairing.
...
1. Нужна ли обязательная HTTPS-защита, несмотря на сложность доверия к self-signed сертификату в обычном браузере?
